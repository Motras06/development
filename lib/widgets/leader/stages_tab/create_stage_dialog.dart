...
class CreateStageDialog {
  static Future<void> show(
    ...
    final nameController = TextEditingController(
      text: stageToEdit?['name'] ?? '',
    );
    final descController = TextEditingController(
      text: stageToEdit?['description'] ?? '',
    );
    ...
    List<Map<String, dynamic>> materialResources =
        List<Map<String, dynamic>>.from(
          stageToEdit?['material_resources'] ?? [],
        );
    List<Map<String, dynamic>> nonMaterialResources =
        List<Map<String, dynamic>>.from(
          stageToEdit?['non_material_resources'] ?? [],
        );
    ...
    Future<void> _pickAndUploadFile(StateSetter setDialogState) async {
      try {
        ...
        final path =
            'stages/$projectId/${DateTime.now().millisecondsSinceEpoch}_$fileName';
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(SnackBar(content: Text('Файл загружен: $fileName')));
        }
    ...
    void addResource(bool isMaterial) {
      ...
      showDialog(
        ...
        builder: (ctx) => AlertDialog(
          title: Text(
            isMaterial ? 'Материальный ресурс' : 'Нематериальный ресурс',
          ),
            ...
            TextButton(
              onPressed: () {
                ...
                final resource = {
                  ...
                  if (isMaterial && qtyCtrl.text.isNotEmpty)
                    'quantity': double.tryParse(qtyCtrl.text) ?? 1,
                  if (isMaterial && unitCtrl.text.isNotEmpty)
                    'unit': unitCtrl.text.trim(),
                };
                (isMaterial ? materialResources : nonMaterialResources).add(
                  resource,
                );
                Navigator.pop(ctx);
              },
              ...
              child: Column(
                ...
                children: [
                  ...
                  ElevatedButton.icon(
                    ...
                    label: Text(
                      _isUploading ? 'Загрузка...' : 'Прикрепить файл',
                    ),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: Theme.of(
                        context,
                          ...
                          : () async {
                              try {
                                final data = {
                                  ...
                                  'material_resources':
                                      materialResources.isEmpty
                                      ? null
                                      : materialResources,
                                  'non_material_resources':
                                      nonMaterialResources.isEmpty
                                      ? null
                                      : nonMaterialResources,
                                };
